...
export default function ProjectsPage() {
  return (
    <>
      ...
      <section className="mx-auto max-w-7xl px-6 pt-16 pb-12 lg:px-10 lg:pt-24">
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-12 lg:items-end">
          ...
          <Reveal
            variant="up"
            delay={360}
            className="flex flex-wrap items-center gap-2 lg:col-span-5 lg:justify-end"
          >
            {filters.map((f, i) => (
              <button
                key={f}
          ...
          {projects.map((p, i) => (
            ...
            >
              <Link href="#" className="block">
                <div
                  className={`relative aspect-[16/10] overflow-hidden rounded-3xl ${p.color} hover-zoom`}
                >
                  <Image
                    src={p.image}
                  ...
                  />
                  <div className="absolute right-4 top-4 inline-flex items-center gap-1 rounded-full bg-background/90 px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest backdrop-blur transition-transform group-hover:-translate-y-0.5">
                    Case study
              ...
              <div className="mt-6 flex flex-col gap-4">
                <div className="flex items-baseline justify-between gap-4">
                  <h2 className="font-serif text-3xl leading-tight tracking-tight md:text-4xl transition-colors group-hover:text-brand-coral">
                    {p.title}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {p.tags.map((t) => (
                    <span
                      key={t}
                      className="rounded-full border border-border bg-card px-3 py-1 font-mono text-[11px] uppercase tracking-widest text-muted-foreground transition-colors hover:border-brand-coral/40 hover:text-brand-coral"
                    >
                      {t}
                    </span>
                  ))}
      ...
      {/* TESTIMONIAL */}
      <section className="mx-auto max-w-7xl px-6 pb-24 lg:px-10">
        <Reveal as="figure" variant="up" className="rounded-3xl bg-foreground p-10 text-background md:p-16">
          <Reveal
            as="p"
            variant="up"
            delay={120}
            className="font-mono text-xs uppercase tracking-widest text-background/70"
          >
            [ A kind word ]
          </Reveal>
          <Reveal
            ...
            variant="up"
            delay={220}
            className="mt-6 font-serif text-3xl leading-tight tracking-tight text-balance md:text-5xl"
          >
          ...
          <Reveal
            ...
            variant="up"
            delay={320}
            className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-background/15 pt-6 text-sm"
          >
            <div>
              <p>Lena Park</p>
            ...
            <Link
              href="/contact"
              className="group inline-flex items-center gap-2 rounded-full bg-background px-5 py-2.5 text-sm text-foreground transition-colors hover:bg-brand-coral hover:text-background"
            >
              Work with us
